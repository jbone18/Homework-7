...
public:
	...
	void set_departure_time(int departure_time) { this->departure_time = departure_time; }
	int get_num_served() { return num_served; }
	int get_total_wait() { return total_wait; }
		...
		if (!the_queue.empty()) 
		{
			...
			else 
			{
				...
				if (clock - plane->ready_takeoff_time > departure_time) 
				{
					...
					// FIXME: calculate the wait time in departure queue
